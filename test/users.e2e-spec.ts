...
import { InsufficientBalanceError, UserNotFoundError } from '../src/users/errors/user-domain.errors';
...
describe('UsersController (e2e)', () => {
  ...
  it('performs balance read and debit flow', async () => {
    const initialResponse = await app.inject({ method: 'GET', url: '/users/1' });
    expect(initialResponse.statusCode).toBe(200);
    expect(JSON.parse(initialResponse.payload)).toMatchObject({ id: 1, balance: 200 });

    const debitResponse = await app.inject({ method: 'POST', url: '/users/1/debit', payload: { amount: 50 } });
    expect(debitResponse.statusCode).toBe(201);
    expect(JSON.parse(debitResponse.payload)).toMatchObject({ id: 1, balance: 150 });

    const finalResponse = await app.inject({ method: 'GET', url: '/users/1' });
    expect(finalResponse.statusCode).toBe(200);
    expect(JSON.parse(finalResponse.payload)).toMatchObject({ id: 1, balance: 150 });
  });

  it('validates debit payload', async () => {
    const response = await app.inject({ method: 'POST', url: '/users/1/debit', payload: { amount: 0 } });
